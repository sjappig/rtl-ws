...
void log_data_rate(const cmplx_u8* complex_signal, int len)
{
    ...
    {
        DEBUG("Current sample rate: %llu samples / %d ms : %llu kHz\n", samples, diff_ms, samples/diff_ms);
        samples = 0;
    }
}
...
void estimate_spectrum(const cmplx_u8* complex_signal, int len)
{
    ...
    for (i = 0; i < blocks; i++)
    {      
        if (add_spectrum(spectrum, &complex_signal[i*FFT_POINTS], power_spectrum, FFT_POINTS, linear_energy_gain) != FFT_POINTS) 
        {
...
int write_spectrum_message(char* buf, int buf_len)
{
    ...
    if (spectrum_averaging_count > 0) 
    {
        ...
        {
            int m = 10*log10(abs(power_spectrum_local[idx] / spectrum_averaging_count_local));
            m = m >= 0 ? m : 0;
            m = m <= 255 ? m : 255;
            buf[len++] = (char) m;
        }
...
int main(int argc, char **argv)
{
    ...
    struct rtl_dev* dev;
    int spectrum_callback_id = -1;
    int logger_callback_id = -1;
    char cert_path[1024];
    char key_path[1024];
    int n = 0;
    int use_ssl = 0;
    ...
    logger_callback_id = add_signal_callback(log_data_rate);
    spectrum_callback_id = add_signal_callback(estimate_spectrum);
    ...
    force_exit = 1;
    remove_signal_callback(spectrum_callback_id);
    remove_signal_callback(logger_callback_id);
    stop_signal_source();
